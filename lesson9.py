...
def about():
    ...
    back_button.pack()
def about_autor():
    about_autor_window = tk.Tk()
    about_autor_window.geometry("300x70+700+550")
    about_autor_window.resizable(False, False)
    about_autor_label = tk.Label(about_autor_window,
    text="Hello, I'm Sasha and I'm 12 years old")
    about_autor_label.pack()
    def back():
        about_autor_window.destroy()
    back_button = tk.Button(about_autor_window, text="Back", command=back, width=10)
    back_button.pack()
def favorite_game():
    favorite_game_window = tk.Tk()
    favorite_game_window.geometry("300x70+700+550")
    favorite_game_window.resizable(False, False)
    favorite_game_label = tk.Label(favorite_game_window,
    text="My favorite game is Minecraft")
    favorite_game_label.pack()
    def back():
        favorite_game_window.destroy()
    back_button = tk.Button(favorite_game_window, text="Back", command=back, width=10)
    back_button.pack()

root = tk.Tk()
root.geometry("600x400+500+400")
root.title("Дневники разработчика")
...
info_menu.add_command(label='About autor', command = about_autor)
info_menu.add_command(label='Favorite game', command = favorite_game)
