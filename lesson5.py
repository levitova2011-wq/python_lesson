...
details_survive_price = {"shield": 50, "energy shield": 80, "resist": 30, "evasion": 100,
"armor": 140, "doshirak": 100, "mivina": 120}
...
def player_maker(player_list):
    user_choose = "yes" #Создается переменная user_choose, которой присваивается значение yes.
    while user_choose == "yes" or user_choose == "y":
        '''начинается цикл while,условием выхода из которого является переменная user_choose'''
        user_input = input("Input a Player name – ")
        if len(user_input)<3:
            continue
            '''В самом цикле объявляется переменная user_input,
            которая после введения имени пользователем проверяется на корректность ввода и добавляется в список
            игроков'''
        player_list.append(str.capitalize(user_input))  #Следующий шаг – сделать запрос на увеличение количества игроков
        user_choose = input("More players – Y/N – ")
    print(f"Players – {str(player_list)[1:-1]}")  #Когда список сформирован, выводим его на экран
